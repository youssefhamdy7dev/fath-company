...
class StoreTruckRequest extends FormRequest
{
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            foreach ($fruits as $index => $fruit) {
                /**
                 * 2️⃣ Validate NO duplicates of (fruit_id + box_type)
                 * This enforces the DB UNIQUE(truck_id, fruit_id, box_type)
                 */
                // Skip rows with missing required fields (Laravel will handle those)
    ...
    public function messages(): array
    {
        return [
            ...
            'total_boxes.required' => 'إجمالى عدد البرانيك مطلوب.',
            'freight.required' => 'النولون مطلوب.',
            'driver_id.exists' => 'السائق المحدد غير موجود.',
            'client_id.exists' => 'العميل المحدد غير موجود.',

            'fruits.required' => 'يجب إضافة صنف واحد على الأقل.',
            'fruits.min' => 'يجب إضافة صنف واحد على الأقل.',

            'fruits.*.fruit_id.required' => 'اختر الصنف.',
            'fruits.*.fruit_id.exists' => 'الصنف المحدد غير موجود.',
            'fruits.*.box_type.required' => 'اختر نوع أو حجم البرنيكة.',
            'fruits.*.box_type.in' => 'نوع البرنيكة غير صالح.',
            'fruits.*.number_of_boxes.required' => 'أدخل عدد البرانيك.',
        ];
    }
}
