...
@section('content')
    @php
        $formatted = $driver->created_at->translatedFormat('التاريخ: ( l ، j F Y ) - الوقت: ( h:i A )');
    ...
    <div class="card shadow-sm p-4 align-items-center align-content-center text-center">
        <div class="d-flex justify-content-end w-100 mb-3">
            <a href="{{ route('drivers.index') }}" class="btn btn-info">
                العودة إلى قائمة السائقون <i class="ps-1 bi-person-video2"></i>
            </a>
        </div>

        <h1 class="mb-4">بيانات السائق</h1>

        <div class="w-75 mx-auto">
            <div class="card p-4 card-background">
                <h5 class="text-muted mb-1">الإسم:</h5>
                <p class="fs-5 fw-semibold">{{ $driver->name }}</p>

                <h5 class="text-muted mb-1">رقم الهاتف:</h5>
                <p class="fs-5 fw-semibold">{{ $driver->phone ?? 'لم يتم تسجيل رقم هاتف' }}</p>
        ...
        <div class="d-flex justify-content-center gap-2 mt-4">
            <a href="{{ route('drivers.edit', $driver->id) }}" class="btn btn-warning px-4">تعديل <i
                    class="bi bi-pencil-square"></i></a>
            <button type="button" class="btn btn-danger px-4" data-bs-toggle="modal" data-bs-target="#deleteModal">حذف<i
                    class="bi bi-trash"></i></button>
        </div>

        <form id="deleteForm" action="{{ route('drivers.destroy', $driver->id) }}" method="POST">
            @csrf
            @method('DELETE')
        </form>
    </div>

    <x-modal id="deleteModal" title="تأكيد الحذف" body="هل أنت متأكد من حذف هذا السائق نهائيًا؟" confirmText="تأكيد"
