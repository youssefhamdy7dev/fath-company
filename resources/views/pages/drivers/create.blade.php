...
@section('content')
    <div class="card shadow-sm p-4 align-items-center align-content-center text-center mt-3">
        ...
        <h1 class="mb-4 text-center">إضافة سائق جديد</h1>

        <form method="POST" action="{{ route('drivers.store') }}" dir="rtl" class="form-control" autocomplete="off">
            @csrf
            <div class="container w-50">
                <x-form.input id="name" label="إسم السائق" name="name" value="{{ old('name') }}" />
                ...
                <div class="text-center mt-4 mb-2">
                    <button type="submit" class="btn btn-reload btn-primary w-50">
                        إضافة السائق <i class="ps-1 bi bi-plus-circle"></i>
                    </button>
                </div>
            </div>
        </form>
